...
class ProductListCreateAPIView(generics.ListCreateAPIView):
    ...
    # Los más recomendable es utilizar la función get_queryset por temas de validación y escalabilidad
...
class ProductUpdateAPIView(generics.UpdateAPIView):
    ...
    def patch(self, request, pk=None):
        ...
        return Response({'message': 'El producto con estos datos no existe'}, status=status.HTTP_404_NOT_FOUND)
...
class ProductRetrieveUpdateDestroyAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self, pk=None):
        if pk is None:
            ...
            return (
                self.get_serializer()
                .Meta.model.objects.filter(state=True, id=pk)
                .first()
            )
